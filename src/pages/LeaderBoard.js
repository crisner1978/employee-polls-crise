    ...
    <div className='mx-auto h-full max-w-5xl px-4 md:px-8'>
      <div className='sm:flex sm:items-center'>
        <div className='pt-16 sm:flex-auto'>
          <h1 className='text-3xl font-semibold text-slate-900'>Leader Board</h1>
              ...
              <table className='min-w-full divide-y divide-gray-300'>
                ...
                <tbody className='divide-y divide-gray-200 bg-white'>
                  {users.sort(getRankDesc).map((user, i) => (
                    <tr
                      key={user.id}
                      className={`${authUser.uid === user.id ? 'bg-blue-50/70' : 'bg-white'}`}>
                      <td className='relative whitespace-nowrap py-4 pr-3 pl-12 text-sm'>
                        <div className='flex items-center'>
                          <div className='h-10 w-10 flex-shrink-0'>
                            <img
                              ...
                              alt={user?.name}
                              className={`${
                                authUser.uid === user.id ? 'ring-4 ring-green-400' : 'bg-white'
                        ...
                        <span className='absolute top-6 left-6 text-sm font-bold'>{i + 1}</span>
                      </td>
                      ...
                      <td className='tableData'>
                        <span>{getCreatedCount(user) + getAnswerCount(user)}</span>
                      </td>
                    </tr>
                  ))}
                </tbody>
